...
const auditRequestContextPrompt = `You are a chatbot assistant to be used for clinical audits. You will follow the following example structure for the audit request conversation: 
\n\nChatbot:
...
app.post("request", (request, response, next) => {
  ...
  openaiClient.chat.completions
    .create({
      messages: [
        { role: "chatbot", content: auditRequestContextPrompt },
    ...
    .then((aiResponse) => {
      response.status(201).send(aiResponse.data.choices[0]);
    });
});
